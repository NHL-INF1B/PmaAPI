...
if (isset($arr)) {
   //  $planning = htmlentities($arr['planning']);
    ...
    //Validate fields
    if ($error = validateFields($planning, $id, $week, $activiteit, $project_id)) {
        echo json_encode($error);
    } else {
        //Verify email does not exist
        if ($error = checkEmailInDataBase($conn, $planning)) {
            echo json_encode($error);
        } else {
           // $planning = "";
            $id = "";
            ...
            $query = "INSERT INTO shedule_line (id, week, activiteit, project_id) VALUES (?,?,?,?)";
    
            //Sending data to the database
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "sisi", $planning, $week, $activiteit, $project_id);
